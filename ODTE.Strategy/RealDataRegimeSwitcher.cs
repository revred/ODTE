...
namespace ODTE.Strategy
{
    ...
    public class RealDataRegimeSwitcher
    {
        ...
        private TwentyFourDayPeriodResult AnalyzeTwentyFourDayPeriod(DateTime startDate, DateTime endDate, int periodNumber)
        {
            ...
            var dailyResults = new List<RegimeDailyResult>();
        ...
        private RegimeDailyResult AnalyzeTradingDay(DateTime date, TwentyFourDayPeriodResult period)
        {
            var result = new RegimeDailyResult { Date = date };
        ...
        private Regime ClassifyRegimeFromRealData(MarketConditions conditions)
        {
            ...
            var realizedVsImplied = conditions.RealizedVolatility / Math.Max(0.1, conditions.ImpliedVolatility);
        ...
        private double SimulateStrategyPnL(string strategy, MarketConditions conditions, Regime regime)
        {
        ...
        private MarketConditions CreateMarketConditionsFromRealData(DateTime date, DailyMarketData data)
        {
            ...
            conditions.RealizedVolatility = Math.Max(0.5, Math.Min(1.5, dailyRange * 100)); // Rough approximation
            conditions.ImpliedVolatility = data.VIX; // Use VIX as proxy for IV

            // IV Rank estimation based on VIX relative to range
            conditions.IVRank = Math.Max(0, Math.Min(100, (data.VIX - 10) / 0.5)); // Rough VIX to IV rank conversion

            // Term structure slope
            conditions.TermStructureSlope = 1.0; // Neutral term structure (no VIX9D data)

            return conditions;
        }

        private MarketConditions CreateSyntheticMarketConditions(DateTime date)
        {
            ...
            conditions.IVRank = random.NextDouble() * 100;
            conditions.TermStructureSlope = 0.8 + random.NextDouble() * 0.4;
            conditions.TrendScore = (random.NextDouble() - 0.5) * 2;
            conditions.RealizedVolatility = 0.7 + random.NextDouble() * 0.8;
