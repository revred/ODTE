namespace ODTE.Strategy.Tests
{
    ...
    public class PM250_ComprehensiveGeneticOptimizer
    {
        ...
        private DailyTradingResult SimulateTradingDay(HistoricalTradingDay day, ComprehensiveChromosome chromosome, decimal currentRFibLimit)
        {
            ...
            if (day.MarketRegime == "VOLATILE")
                regimeMultiplier = chromosome.VolatileMarketMultiplier;
            else if (day.MarketRegime == "CRISIS")
                regimeMultiplier = chromosome.CrisisMarketMultiplier;
            else if (day.MarketRegime == "BULL")
        ...
        private int CalculateNotchMovement(decimal monthlyPnL, decimal winRate, ComprehensiveChromosome chromosome, decimal currentLimit)
        {
            ...
            if (monthlyPnL < 0 && movement == 0)
            {
                ...
                if (adjustedLoss >= chromosome.CatastrophicLossThreshold)
                    ...
                    movement = 2;
                else if (adjustedLoss >= chromosome.MinorLossThreshold)
                    movement = 1;
                else
                    movement = 0;
            }
        ...
        private int CalculateExpectedTrades(HistoricalTradingDay day)
        {
            ...
            if (day.MarketRegime == "CRISIS")
                baseTrades = 8;    // Fewer trades in crisis
            else if (day.MarketRegime == "VOLATILE")
                baseTrades = 12;   // Moderate trading
            else if (day.MarketRegime == "BULL")
                baseTrades = 15;   // More opportunities
            else if (day.MarketRegime == "BEAR")
        ...
        private decimal CalculateExpectedWinRate(HistoricalTradingDay day, ComprehensiveChromosome chromosome)
        {
            ...
            if (day.MarketRegime == "CRISIS")
                regimeAdjustment = -0.15m;
            else if (day.MarketRegime == "VOLATILE")
                regimeAdjustment = -0.08m;
            else if (day.MarketRegime == "BULL")
        ...
        {
            decimal baseVix;
            if (date.Year >= 2008 && date.Year <= 2009)
                baseVix = 35m;
            else if (date.Year == 2020 && date.Month >= 2 && date.Month <= 4)
                baseVix = 45m;
            else if (date.Year == 2022)
                baseVix = 28m;
            else if (date.Year >= 2024)
                baseVix = 25m;
            else if (date.Year == 2018 && date.Month >= 2 && date.Month <= 3)
                baseVix = 32m;
            else
                baseVix = 18m;
