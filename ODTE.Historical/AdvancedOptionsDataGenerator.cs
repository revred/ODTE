...
/// </summary>
public class AdvancedOptionsDataGenerator : IDataSource
{
    ...
    public AdvancedOptionsDataGenerator()
    {
    ...
    private async Task<MarketDataBar> GenerateMinuteBar(DateTime timestamp, string symbol, double baseVol, MarketRegime regime)
    {
        ...
        {
            Timestamp = timestamp,
            Open = price * 0.9995, // Realistic intrabar variation
            High = price * (1.0 + Math.Max(0, jumpComponent) * 0.1),
            Low = price * (1.0 - Math.Max(0, -jumpComponent) * 0.1),
            Close = price,
